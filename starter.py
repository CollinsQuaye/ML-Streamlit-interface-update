...
with col1:
    st.markdown('##')
with col2:
    st.image('Images/met.png', width=250)
with col3:
    st.markdown('##')
...
with st.expander('Unit Casing Input', expanded=True):
    ...
    if submit_button:
        ...
        d = {'Holding Capacity (Tonnes)':[capacity], 'Door Opening Height (mm)':[door_height],'Door Opening Width (mm)': [door_width], 
        ...
        pred_data = pred_model('final_model')
